function [Tout, Qout] = mine_geothermal_ensemble(k_r,Cp_r,rho_r,diam,...
    Tr)
...
Tf_ini = 7;        % inflow temperature (degC) 
nyrs   = 0.2;       % flow duration (yrs)
head   = 5.2e-12;    % hydraulic head loss through mine (m). e.g. 6.3e-12 
...
[Tn Tp]= mine_heat(t, r, L, v, np, nn, no, Tf_ini, k_r, Cp_r, rho_r,...
    Tr, npipes, node_pipes_out, pipe_nodes, weighted_flow_in);
...
%         grid on
%         colormap(jet)
%         minQ = min(Q); maxQ = max(Q); dQ = maxQ-minQ;
%         if (dQ/maxQ<0.01)
%             eps = 0.01*maxQ;
%         else
%             eps = 0;
